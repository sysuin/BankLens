...
| Tests | **283 passed**, 163 test functions in 12 files, 10.2 s | **411 passed** in 22 files, ≈55–85 s (boots a throwaway Postgres) | `python -m pytest -q` |
...
| Local-model golden pass rate | n/a | **4/4 blocking checks at 6/6; advisory 2/6; zero validator retries; p50 34.3 s; $0** (re-run after the Phase 8 prompt, catalogue and validator changes; Phase 4 table for the first run) | `make compare` |
| Harbor golden pass rate, hosted | n/a (Meridian only) | **6/6 on every blocking check, zero validator retries, p50 6.6 s, $0.0102 per query** (first run: 4/6 on retrieval support at $0.0186; see Phase 8 findings for the two bugs and the two cause fixes) | `make evals PROVIDER=openai TENANT=harbor` |
...
- **The miss, recorded and then fixed at the cause (9 Sep).** Two Harbor cases failed `retrieval_supports_recommendation`: for a deficit or thin-saver customer retrieval ranked the auto loan and the credit card, the guardrail rightly ruled those out, and the model picked the everyday account, whose chunk was never retrieved. It learned that name from the validator's retry message, which listed the whole shelf. I first tried a deterministic "customer need" clause in the retrieval query; it is a lexical query, so "credit" and "secured" pulled the credit card and the mortgage into Meridian's top four and the local model offered a personal loan to a deficit customer. Reverted. Two cause fixes instead: (1) Harbor's catalogue gained the product it lacked, a Fresh Start Consolidation Loan written for the deficit profile, and the everyday account's document now says who it is for; (2) inside `build_profile()` the validator hints only at the retrieved products and rejects a primary recommendation that retrieval never surfaced (a secondary may be any catalogue product, because a cross-sell from the same shelf is not a hallucination). Result: **Harbor 6/6 on every blocking check, zero retries, $0.0102 per query at p50 6.6 s** (from 4/6, $0.0137, 10.9 s); Meridian unchanged at 6/6, $0.0100, zero retries.
