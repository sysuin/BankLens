...
def run_once(path: Path, vector_store) -> dict:
    ...
    row["stages"]["categorize"] = {
        "s": dt,
        "in": p,
        "out": c,
        "usd": cost("gpt-4o-mini", p, c),
    }
    ...
    row["risk"] = (
        metrics.risk_profile.value
        if hasattr(metrics.risk_profile, "value")
        else str(metrics.risk_profile)
    )
    row["products"] = (
        [profile.recommended_product_1, profile.recommended_product_2]
        if hasattr(profile, "recommended_product_1")
        else None
    )
    return row
...
def main() -> int:
    ...
    for r in range(args.repeats):
        for path in SAMPLES:
            try:
                ...
                runs.append(row)
                print(
                    f"{path.name:<32} {row['total_s']:6.1f}s  ${row['total_usd']:.4f}  rows={row['rows']}"
                )
    ...
    summary = {
        ...
        "usd_per_statement": {
            "p50": pct(usd, 0.5),
            "mean": statistics.fmean(usd),
            "max": max(usd),
        },
        "stage_p50_s": {
            s: pct([r["stages"][s]["s"] for r in ok], 0.5) for s in stage_names
        },
        "stage_share_of_cost": {
            s: sum(r["stages"][s].get("usd", 0) for r in ok) / max(sum(usd), 1e-9)
            for s in stage_names
        },
        "tokens_p50": {
            s: {
                "in": pct([r["stages"][s].get("in", 0) for r in ok], 0.5),
                "out": pct([r["stages"][s].get("out", 0) for r in ok], 0.5),
            }
